  ...
  template: `
    <section class="table-card">
      <div class="table-toolbar">
        ...
        <button class="btn primary" type="button" (click)="add.emit(); toast.info('فرم افزودن باز شد')">+ افزودن</button>
      </div>
      ...
      <div class="desktop-table" *ngIf="!loading && filtered().length">
        <table><thead><tr><th *ngFor="let c of columns">{{ c.label }}</th><th>عملیات</th></tr></thead>
          <tbody><tr *ngFor="let row of paged()"><td *ngFor="let c of columns">{{ row[c.key] }}</td><td class="actions"><ng-container *ngTemplateOutlet="actionButtons; context: {$implicit: row}"></ng-container></td></tr></tbody>
        </table>
      </div>
      <div class="mobile-cards" *ngIf="!loading && filtered().length">
        <article *ngFor="let row of paged()" class="mobile-row-card">
          ...
          <button class="btn ghost" type="button" (click)="selectedRow.set(row)">عملیات</button>
        </article>
      </div>
