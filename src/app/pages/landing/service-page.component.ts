...
@Component({
  ...
  template: `
    <main class="clinic-site service-detail" dir="rtl">
      <section class="service-hero reveal-section">
        <div><a routerLink="/" class="back-link">← بازگشت به خانه</a><span class="eyebrow">Dental Service</span><h1>{{ title() }}</h1><p>یک لندینگ اختصاصی با ساختار شفاف، تصویر بزرگ، محتوای قابل اسکن و CTA ثابت برای رزرو نوبت.</p><a class="premium-btn primary" href="#service-booking">رزرو مشاوره {{ title() }}</a></div>
      </section>
      ...
      <section class="premium-section reveal-section"><div class="section-heading"><span class="eyebrow">Treatment Journey</span><h2>مراحل درمان</h2></div><div class="steps-grid"><div *ngFor="let step of steps; let i = index"><strong>0{{ i + 1 }}</strong><h3>{{ step }}</h3><p>هر مرحله با توضیح کامل، کنترل کیفیت و هماهنگی تیم درمان انجام می‌شود.</p></div></div></section>
      <section class="service-content-grid reveal-section"><article><span class="eyebrow">مزایا</span><h2>چرا در کلینیک دکتر مقدم؟</h2><ul><li>طراحی دیجیتال و نتیجه قابل پیش‌بینی</li><li>مواد و تجهیزات باکیفیت</li><li>فضای مینیمال و آرام</li><li>پیگیری بعد از درمان</li></ul></article><article><span class="eyebrow">نمونه‌کارها</span><h2>نتایج واقعی</h2><p>گالری قبل و بعد درمان‌ها با تمرکز بر طبیعی بودن فرم لبخند، رنگ و هماهنگی صورت طراحی شده است.</p></article></section>
      <section class="premium-section reveal-section"><div class="section-heading"><span class="eyebrow">FAQ</span><h2>سوالات متداول</h2></div><div class="faq-list"><details *ngFor="let faq of faqs"><summary>{{ faq.q }}</summary><p>{{ faq.a }}</p></details></div></section>
      <section id="service-booking" class="final-cta reveal-section"><h2>رزرو نوبت {{ title() }}</h2><p>برای دریافت برنامه درمانی اختصاصی، مشاوره خود را ثبت کنید.</p><div class="hero-actions"><a class="premium-btn primary" href="tel:+982100000000">تماس فوری</a><a class="premium-btn glass" href="https://wa.me/989120000000">واتساپ</a></div></section>
    </main>
  `
})
