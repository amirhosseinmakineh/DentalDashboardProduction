  ...
  template:`<section class="screen-stack consultant-dashboard">
    ...
    <article class="state-card">
      <div class="filter-panel user-filter-grid responsive-filter-panel open">
        <div class="state-card">وضعیت پروفایل: {{ profileId ? 'آماده' : 'نیازمند تکمیل' }}</div>
        <button class="btn ghost" type="button" (click)="resolveProfileId(true)"><i class="fa-solid fa-user-check"></i> بررسی پروفایل</button>
        <button class="btn ghost" type="button" (click)="loadLeads()"><i class="fa-solid fa-rotate"></i> به‌روزرسانی لیدها</button>
      </div>
      <p *ngIf="!profileId" class="state-card">برای فعال شدن دکمه‌ها ابتدا از منوی <a routerLink="/consultant/profiles">تکمیل پروفایل مشاور</a> اطلاعات خود را کامل کنید.</p>
...
export class ConsultantAttendancePage implements OnInit, OnDestroy {
  ...
  loadLeads(showError=true){if(!this.profileId){this.leads.set([]);return;}const params=new HttpParams().set('profileId',this.profileId).set('pageNumber',1).set('pageSize',50);this.http.get<PagedResponse<LeadAssignmentItem>>(`${this.apiBase}/Consultant/GetLeads`,{params}).subscribe({next:r=>this.leads.set(this.extractLeadItems(r)),error:()=>showError&&this.toast.error('دریافت لیدهای مشاور ناموفق بود')});}
  resolveProfileId(showMessage=false){const stored=currentConsultantProfileId();if(stored){this.profileId=stored;return;}const phone=currentUserPhone();if(!phone){if(showMessage)this.toast.error('اطلاعات حساب شما کامل نیست؛ ابتدا پروفایل مشاور را تکمیل کنید');return;}const params=new HttpParams().set('phoneNumber',phone).set('pageNumber',1).set('pageSize',1);this.http.get<any>(`${this.apiBase}/Consultant/GetConsultants`,{params}).subscribe({next:r=>{const items=Array.isArray(r)?r:(r.items??r.data??[]);const id=extractConsultantProfileId(items);if(id){this.profileId=id;persistConsultantProfileId(id);this.loadLeads(false);if(showMessage)this.toast.success('پروفایل مشاور آماده شد');}else if(showMessage)this.toast.error('برای این شماره موبایل پروفایل مشاور پیدا نشد؛ ابتدا تکمیل پروفایل را انجام دهید');},error:()=>showMessage&&this.toast.error('بررسی پروفایل مشاور ناموفق بود')});}
  private extractLeadItems(value:unknown):LeadAssignmentItem[]{if(Array.isArray(value))return value as LeadAssignmentItem[];if(!value||typeof value!=='object')return [];const record=value as Record<string,unknown>;for(const key of ['items','Items','data','Data','result','Result','item','Item','list','List']){const nested=this.extractLeadItems(record[key]);if(nested.length)return nested;}return [];}
