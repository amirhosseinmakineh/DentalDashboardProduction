...
export class PatientComponent { toast=inject(BaseToastService); private router=inject(Router); reservations=reservations; rating=signal(false); cols=[{key:'date',label:'تاریخ'},{key:'time',label:'ساعت'},{key:'consultant',label:'مشاور'},{key:'status',label:'وضعیت'}]; filters=[{key:'status',label:'وضعیت',type:'select' as const,options:['تاییدشده','در انتظار']}]; section(){return this.router.url.includes('/reservations')?'reservations':this.router.url.includes('/rating')?'rating':'profile'} pageTitle(){return this.section()==='profile'?'پروفایل بیمار':this.section()==='reservations'?'رزروهای من':'امتیازدهی'} pageDescription(){return this.section()==='profile'?'مشاهده اطلاعات پرونده و درمان فعال.':this.section()==='reservations'?'مشاهده وضعیت نوبت‌ها و رزروها.':'ثبت بازخورد و امتیاز برای مشاور.'} save(){this.toast.success('امتیاز شما ثبت شد'); this.rating.set(false)} }
